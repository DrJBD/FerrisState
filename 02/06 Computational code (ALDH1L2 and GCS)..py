...
from Glycine Cleavage System import mito_gcs
...
def mito_fdh_dehydrogenase(pool, step, step_timespan):
    ...
    10-formylTHF + NADP+ -> CO2 + NADPH
    Irreversible two-substrate Michaelis-Menten
    """
    print(' ', sys._getframe().f_code.co_name)
    print('    chothf:', pool[CHOTHF], ' nadpp:', pool[NADPP], ' co2:', pool[CO2], ' nadph:', pool[NADPH])

    # Parameters
    Vmax = 774      # µM / hour
    Km1_CHOTHF = 3.2    # µM
    Km_NADP = 2.0       # µM [8] Kinetic value from ALDH1L1 (1.5-2.0 µM)
